  ...
  useEffect(() => {
    const fetchData = async () => {
      const response =
        sort !== ""
          ? await fetch(
              `https://gray-charming-viper.cyclic.app/record/meals/${sort}?ascending=${ascending}`
            )
          : await fetch(`https://gray-charming-viper.cyclic.app/record/meals`);
      if (!response.ok) {
    ...
    useEffect(()=>{
      ...
      newOrder()
    },[order, context, setOrder,records])
  return (
    <>
    {records.length > 0 ? <article className="min-h-full py-5">
      ...
      <div className="font-normal text-white md:text-left text-center text-shadow-md md:px-24">
          Only maximum of 20 allowed per item*
        </div>
      <div className="grid xl:grid-cols-3 md:grid-cols-2 grid-flow-row auto-rows-auto place-items-center gap-10 px-5 pb-10 my-5 w-full">
        {records !== [] &&
          records.map((record) => {
            return (
              <div
    ...
    <article className="min-h-full py-5 px-5 flex flex-col items-center gap-20">
      <h1 className="text-3xl text-white">Please wait a moment</h1>
      <h1 className="text-3xl text-white">Fetching Data....</h1>
      <img src="https://static.thenounproject.com/png/3095076-200.png" alt="" className="animate-spin" />
      <button className="btn !lg:w-1/2" onClick={()=>{window.location.reload()}}>Taking too long?</button>
    </article>
  }
  </>
  );
